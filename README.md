...
- **MCP server** -- Model Context Protocol endpoint at `POST /mcp` (Streamable HTTP transport) that exposes governed actions and FGA-scoped read queries as MCP tools for external AI agents (Claude Code, Cursor, Anthropic SDK). An agent is just another OIDC principal calling the same 8-stage action pipeline. Capability-gated per pack. See [`docs/mcp-server.md`](docs/mcp-server.md).
