...
2026-08-21 | loop-0821-a7c3 | FIXING-BUILD computed-field-evaluator | partial-progress | call site :436 missing schema field ComputeContext requires; mirrored sibling :398
2026-08-22 | loop-0822-4f1a | mmdp/openapi-mounted-route-coverage | shipped | spec was schema-derived only: 247 mounted paths (attachments, geo, timeseries, workflow, marking) were live but undocumented; completed from the Express router at request time, 321→568 paths, wildcards listed not dropped
2026-08-22 | loop-0822-4f1a | mmdp/object-store-blob-adapter | shipped | @altius/storage-s3 lifts the bytea ceiling on attachments; tenant is a key segment (traversal-guarded), creds via SDK chain not config; verified end-to-end against a local S3 stub through the real route
2026-08-22 | loop-0822-4f1a | spi-conformance/type-honesty | shipped | `as never` casts in the workshop suite hid a widgetType/type mismatch and an invented VariableSource kind — suite typechecked green while asserting shapes the SPI never declared; casts removed, monorepo typecheck restored
2026-08-22 | loop-0822-4f1a | mmdp/read-in-place-parquet | shipped | external datasets read a parquet file in the object store with no ingest; schema comes from the footer, writes refused 409; live check found the decorator's `{...inner}` spread dropped every prototype method — explicit forwarding class + delegation test now cover it
2026-08-22 | loop-0822-4f1a | spi-conformance/external-source-parity | shipped | conformance for the new externalSource field caught PostgresDatasetService.create returning a dataset without it (row persisted, return value did not) — memory had it, so only running both providers found the divergence
2026-08-22 | loop-0822-4f1a | mcp-server/test-flake | shipped | `await import('@altius/actions')` inside setup() charged the package's module load to the test's default 5s budget; under a full parallel run it hit 5133ms and failed. Hoisted to a static import; full run 41/41 green
2026-08-22 | loop-0822-4f1a | spi-conformance/blob-store-parity | shipped | three BlobStore impls, zero proof they agreed — and they did not: memory deduplicated by content hash with no refcount, so deleting one attachment emptied another with identical bytes and the second upload's filename was lost. Contract settled as one-upload-one-blob; memory brought in line; S3 exercised against a local stub through the real SDK
2026-08-22 | loop-0822-4f1a | storage-postgres/lifecycle-suite-rerun | shipped | provider-lifecycle dropped its tables in beforeAll then relied on applySchema, which no-ops once the version is recorded — every CRUD case failed on a second run against the same database. Suite now forgets its migration row first
