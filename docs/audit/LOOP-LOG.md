...
2026-08-21 | loop-0821-pii1 | aip-llm/pii-obfuscation-pre-model | partial-progress | PII obfuscator on LLM egress path; reuses getVisibleFields (fail-closed); 12 tests; caller-declared sensitiveValues is opt-in — auto-detection + reversible tokenization still open
2026-08-21 | loop-0821-cel1 | aip-agents/no-code-business-rules-over-data-foundry-rul | partial-progress | both CEL fail-open holes closed (dev stub opt-in + inline evaluator fail-closed); 2 new tests proven two-sided; 1045 tests green
2026-08-21 | loop-0821-pii1 | misc-2/user-authored-serverless-functions-typescrip | closed | VERIFY-0 found all sub-gaps closed at HEAD; stale 15 Aug evidence updated; row moved to full + DONE
2026-08-21 | loop-0821-pii1 | aip-agents/managed-multi-provider-llm-gateway-model-acc | partial-progress | VERIFY-0 found 17 Aug evidence stale (4 providers, catalog, quota, embed, vectorSearch all exist); evidence updated; per-tenant enablement + cost allocation still open
2026-08-21 | loop-0821-pii1 | ontology-core/functions-user-authored-code-logic-on-object | partial-progress | VERIFY-0 found 15 Aug evidence stale (Python runtime, MCP function tools, shipped pack all exist); function-backed actions + custom aggregations still open
2026-08-21 | loop-0821-fn1 | ontology-core/functions-user-authored-code-logic-on-object | partial-progress | function-backed actions CLOSED (InvokeFunctionEffect in ActionEffect union; parser+executor+API wiring; structural ActionFunctionExecutor interface; 4 tests two-sided; 1045 tests green); custom aggregations still open
