...
- **`operational-automation`** (`b33f13e`) — Automations are now declarable in pack.yaml (event: objectType + change kinds + optional CEL condition, or schedule: interval) and run a governed action via ActionExecutor under a declared actor. CAVEATS — UNHARDENED, treat as single-pod/dev only: the runner starts on every api pod (event & schedule fire N× on multi-replica; no leader/dedup), the consent step is skipped (no consentPurpose set), setInterval has no non-overlap guard, there is no idempotency for an at-least-once (Redpanda) bus, and the self-loop guard is a substring match (cross-automation cycles unguarded). See the row for the full list.
...
> ✅ **UPDATED 16 Aug 2026 — automation runtime landed (`b33f13e`), UNHARDENED.** absent → partial. Single-pod/dev only until the caveats below are fixed.

**Update (16 Aug 2026 — landed):** Automations are declarable in pack.yaml (`automations: [file...]`). Each manifest has a trigger (`event`: objectType + change kinds + optional CEL `condition`, or `schedule`: interval), an `action { name, params }` with static values or `{ from: <CEL> }` mappings, an `actor { id, roles }`, and (for schedule) a `tenantId`. Runtime: `AutomationRunner` (packages/api/src/automation/) subscribes to the EventBus and, on a matching object lifecycle event, evaluates the CEL condition against `{ event, object, changes }` and runs the action via ActionExecutor under a `type:'system'` actor (ReBAC authz + audit apply); schedule triggers run per interval under the declared tenant. Loaded by schema-loader, started/stopped at boot (server.ts). Tests: parser (event/schedule/validation) + runner (param mapping, CEL gating, self-loop guard, schedule, unknown-action) — 15 tests. KNOWN GAPS (do not deploy multi-pod as-is): (1) the runner starts on **every** api pod, so event & schedule automations fire N× on multi-replica — no leader election/dedup; (2) the consent step is **skipped** (no consentPurpose/subjectId set), unlike the REST/GraphQL/MCP action paths; (3) schedule uses `setInterval` with no non-overlap guard; (4) no idempotency for an at-least-once (Redpanda) bus → duplicate events → duplicate actions; (5) the self-loop guard is a substring match on `causedBy.actor` and does not stop cross-automation cycles; (6) cron is unsupported (interval only); (7) no retry/DLQ/metrics; per-event `getObject` is an N+1 across matching automations.
