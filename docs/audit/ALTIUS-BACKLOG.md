...
> ✅ **EVIDENCE UPDATED (Fase 21).** Added `QrCodeReaderWidget` and `CameraCaptureWidget` (packages/web/src/widgets/components/Fase21Widgets.tsx) using `navigator.mediaDevices.getUserMedia` and canvas capture. REST routes for device capture and deep-link resolution (`packages/api/src/rest/device-capture-routes.ts`).
