...
> ⚠️ **EVIDENCE UPDATED 19 Aug (PR #13, §4C).** The `WidgetLibraryService` SPI and `InMemoryWidgetLibraryService` cited in the Phase 10 evidence were DELETED as redundant — `WorkshopPlatformService` is the surviving app-definition model. The grade stays `partial` because no UI rendering layer exists.

**Evidence (updated 19 Aug, §4C):** The redundant `WidgetLibraryService` SPI (`packages/spi/src/widget-library.ts`) and `InMemoryWidgetLibraryService` (`packages/storage-memory/src/in-memory-widget-library.ts`) were DELETED in §4C (PR #13). They had zero consumers outside index re-exports. The surviving app-definition model is `WorkshopPlatformService` (packages/spi/src/workshop-platform.ts), which defines `WidgetCatalogEntry` with display optimization metadata, app/module/page definitions, variables, and Object Views. `InMemoryWorkshopPlatformService` ships 60 pre-registered widget definitions across 12 categories. 5 tests in phase12-workshop-platform.test.ts. No REST/GraphQL routes for the workshop platform service. No persistent storage. No UI rendering.
...
> ⚠️ **EVIDENCE UPDATED 19 Aug (PR #13, §3.5).** 9 REST endpoints were wired. `record()` is intentionally NOT exposed (instrumentation-only — exposing a manual write endpoint would allow callers to forge usage data). The grade stays `partial` — not wired into the API request pipeline, no PostgreSQL store, no 30-day windowing.

**Evidence (updated 19 Aug, §3.5):** `OntologyUsageMetricsService` SPI (packages/spi/src/usage-metrics.ts) with `record`, `getObjectTypeMetrics`, `getActionFunctionMetrics`, `getSummary`, `queryEvents`, `getActiveUserCount`, and monitoring rule CRUD/evaluation. `OntologyUsageEvent` tracks tenantId, userId, operation type, objectType, objectId, actionOrFunctionName, success, durationMs, and timestamp. `InMemoryOntologyUsageMetricsService` (packages/storage-memory/src/in-memory-usage-metrics.ts) implements full event recording, aggregation, querying, and monitoring rule evaluation. 9 REST endpoints wired in §3.5 (commit `65066d5`): `GET /api/v1/usage-metrics/object-types/:objectType`, `GET /api/v1/usage-metrics/action-functions/:name`, `GET /api/v1/usage-metrics/summary`, `GET /api/v1/usage-metrics/events`, `GET /api/v1/usage-metrics/active-users`, `GET/POST /api/v1/usage-metrics/monitoring-rules`, `DELETE /api/v1/usage-metrics/monitoring-rules/:id`, `POST /api/v1/usage-metrics/monitoring-rules/evaluate`. `record()` is NOT exposed — it is an internal instrumentation hook only. 10 usage metrics tests + 12 route tests in usage-metrics-routes.test.ts.

**Gap:** Not wired into the actual API request pipeline (events must be recorded manually via internal call). No PostgreSQL metrics store. No 30-day rolling window aggregation (queries are ad-hoc). No change-impact analysis. No GraphQL surface.
...
> ⚠️ **EVIDENCE UPDATED 19 Aug (PR #13, §4C).** The Phase 10 evidence cited `InMemoryWidgetLibraryService` (now deleted). The Phase 12 evidence already cited `WorkshopPlatformService` (the surviving model), so the Phase 12 evidence remains accurate. The grade stays `partial`.

**Evidence (Phase 12, re-verified 19 Aug):** `WorkshopPlatformService` SPI (packages/spi/src/workshop-platform.ts) defines `WidgetCatalogEntry` with display optimization metadata (supportsVirtualization, supportsColumnResize, supportsFrozenColumns, supportsDensityModes, defaultPageSize) and mobileOptimized flag. `InMemoryWorkshopPlatformService` ships with 60 pre-registered widget definitions across 12 categories: data (object_table, object_list, object_view, property_list, object_set_title, links, tree_view, kanban), chart (chart_xy, chart_pie, chart_bar, chart_vega, pivot_table, metric_card, waterfall, observability_chart, heatmap, scatter_plot), filter (filter_list, object_selector, date_picker, date_range, search_bar, user_select), input (text_input, number_input, checkbox, radio_group, dropdown), layout (tabs, stepper, markdown, header, spacer, divider, progress_bar, badge, tooltip, accordion), action (button_group, action_form), media (media_preview, media_uploader, pdf_viewer, image_annotation, spreadsheet_display, video_player, audio_player), collaboration (comments, action_log), ai (aip_chat, aip_generated_content), navigation (mobile_navbar, breadcrumb), time (time_series, gantt, timeline, calendar), geo (map, current_location). 5 tests in phase12-workshop-platform.test.ts. The redundant `WidgetLibraryService` cited in earlier Phase 10 evidence was deleted in §4C.
...
> ⚠️ **EVIDENCE UPDATED 19 Aug (PR #13, §3.2).** REST endpoint was wired using the real `DefaultAccessExplanationService` from `packages/security` (not an in-memory fake). The grade stays `partial` — no simulation for other principals, no denial-reason surfacing on reads, no marking policy integration in the explanation.

**Evidence (updated 19 Aug, §3.2):** `AccessExplanationService` SPI with `explain()` that returns `AccessExplanation` containing granted/denied, resource, userId, and detailed `AccessExplanationReason[]` (packages/spi/src/security-governance.ts). `DefaultAccessExplanationService` (packages/security/src/authz/access-explanation-service.ts) implements the service by checking tenant isolation, ReBAC relations (via the live `AuthorizationService.check`), marking policy, and consent, producing a human-readable summary. REST endpoint wired in §3.2 (commit `7bbae51`): `POST /api/v1/access-explanation`. Uses the real `DefaultAccessExplanationService` backed by the live `AuthorizationService`, not an in-memory fake. 13 security-governance route tests (shared with justification and scoped-session routes).

**Gap:** No simulation for another principal (only the calling user). No denial-reason surfacing on reads (still only post-hoc _redactedFields). No marking policy integration in the explanation (placeholder). No GraphQL surface.
...
**Evidence (Phase 13):** `WorkshopBuilder` (packages/web/src/widgets/builder/WorkshopBuilder.tsx) provides a drag-and-drop app builder with: widget event system (click/select/drag events, variable binding, setVariable callback); page navigation and history (URL hash-based deep linking in MobileAppLauncher); module composition (pages, sections, widget instances, layout kinds); dirty tracking and save/export. `WidgetContext` (packages/web/src/widgets/types.ts) carries variables, setVariable, navigate, and event callbacks — the reactive variable system from Phase 1. `AppRenderer` renders app definitions with page switching. 22 builder tests pass.

**Gap:** No URL-encoded shareable state (deep links work for page navigation but not for variable state). No module changelog or rebase (app definitions are in-memory only, not versioned). No persistent app definition storage backend. No module embedding or cross-app communication.
...
**Evidence (Phase 13):** `DigitalTwinCanvasWidget` (packages/web/src/widgets/components/DigitalTwinCanvasWidget.tsx) provides the Vertex canvas with scenario mode: what-if toggle, per-node property overrides (status/metric), scenario override count, reset overrides, and detail panel with override controls. `GraphWidget` provides the graph canvas with click-to-select and neighbor highlighting. The `WorkshopBuilder` provides the scenario pane (widget composition, variable binding, preview mode). The reactive variable system (WidgetContext) supports scenario variables. 8 digital twin tests + 14 graph tests pass.

**Gap:** No dedicated scenario pane UI (what-if is embedded in the digital twin widget, not a separate panel). No Workshop scenario widgets/variables/buttons as first-class widget types. No Control Panel admin settings for scenario configuration. No scenario comparison view (side-by-side baseline vs what-if). No scenario persistence from the UI.
