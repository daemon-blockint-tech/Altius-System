...
Generated from code-verification passes, most recently 15 Aug 2026. **189** capabilities graded: **5 full, 89 partial, 95 absent**.

The 187 rows below are the work items. Three of them now read `full` — the graph-traversal capability, which appears under three theme groupings — and are kept here with their evidence rather than silently removed. The two capabilities that were already `full` (Ontology core semantic model, Audit immutability) are not listed as work items.
...
**Evidence (read 15 Aug):** Prior gap (1) is FIXED: crossReferenceSchema now runs in production. packages/actions/src/parser/index.ts:815-823 exposes crossReferenceManifest, packages/api/src/schema-loader.ts:493-504 wraps it as crossReferenceManifests, and it is called post-merge at schema-loader.ts:1011 with findings logged at :1013-1015 and returned as manifestIssues (:1042). Unknown linkType/objectType/actionType and bad @param refs are therefore reported at boot. The declarative surface is otherwise real end to end: all six effect kinds parse (parser/index.ts:245 VALID_EFFECT_TYPES) and all six dispatch in the executor (packages/actions/src/executor/action-executor.ts:856-871); params are now type-checked against the ODL declaration, not just presence-checked (action-executor.ts:686-701 checkParamType, from 97f468f) and object params are resolved from storage under the caller's RequestContext (:735-745); permissions come from @actionType(permission:) via actionPermissionRelation (packages/api/src/server.ts:1557-1572) and are enforced at action-executor.ts:294-309, with assertActionAuthzCoverage (packages/api/src/config.ts:247-269) fatal for an action that has neither an object @param nor preconditions; side effects run with retry/backoff and honour rollback policy (action-executor.ts:484-497, packages/actions/src/sideeffects/side-effect-executor.ts:112-114); both surfaces expose dryRun (packages/api/src/graphql/resolver-generator.ts:1406, packages/api/src/rest/route-generator.ts:1601). Remaining defects: crossReferenceManifests downgrades every finding, including severity-'error' UNKNOWN_LINK_TYPE, to 'warning' (schema-loader.ts:500), so a typo'd linkType still boots and fails mid-transaction; there is no updateLink effect at all (parser/index.ts:245), so a 'link rule' cannot modify link properties; ROLLBACK_ALL cannot undo a deleteObject effect (action-executor.ts:544-547 explicitly skips 'deleted' objects for want of an SPI restore op), so a delete rule plus a failing webhook under ROLLBACK_ALL reports failure while leaving the object deleted; sideEffects.type is limited to webhook|event (parser/index.ts:582).

**Gap:** Manifest cross-reference is advisory only (all findings demoted to warnings at schema-loader.ts:500), link rules cannot express an update, and a delete rule is not actually rollback-safe under ROLLBACK_ALL. Boot-time validation no longer being absent is the real change.
...
**Evidence (read 15 Aug):** Both prior gaps survive verbatim. (1) No updateLink effect: packages/actions/src/parser/index.ts:245 VALID_EFFECT_TYPES = updateObject, createLink, deleteLink, createObject, deleteObject, recordConsent; packages/actions/src/parser/types.ts:9-86 has no UpdateLinkEffect. (2) ROLLBACK_ALL still cannot undo a delete: packages/actions/src/executor/action-executor.ts:544-547 comments that 'deleted' object rollbacks are best-effort and does nothing, while created/updated objects and both link directions are compensated (:516-543). What 11a6510 added is direct editing, not governed editing: generic update/delete mutations now exist on GraphQL (packages/api/src/graphql/resolver-generator.ts:976, :1044) and REST (PUT and DELETE /api/v1/{plural}/:id at packages/api/src/rest/route-generator.ts:864-865, :951-952) with expectedVersion / If-Match optimistic concurrency. That path is governed only by a single FGA 'editor' check (resolver-generator.ts:986-991, :1054-1059; route-generator.ts ~:885). It runs no consent check (consentService appears only on read paths — resolver-generator.ts:475, :697) and writes NO audit record: deps.auditWriter is invoked only from packages/api/src/consent/router.ts:123,137,145, packages/api/src/relationships/router.ts:155,179,192, packages/api/src/functions/invoke-function.ts:60 and the action pipeline (packages/api/src/server.ts:763), and packages/engine/src/objects/object-manager.ts contains no audit call at all. There is also no write-side field policy — redactFields (resolver-generator.ts:1014) shapes the response only — so an 'editor' can overwrite @sensitive fields unaudited. Object CREATE and all link editing remain action-only: the complete generated Mutation set is update<Type>/delete<Type>, action mutations, object-set CRUD, grant/revokeRelationship, generate/embed, recordConsent (resolver-generator.ts:976-2003), and REST has no POST /api/v1/{plural} (route patterns at route-generator.ts:461,652,758,865,952,1035,1110,1269,1397,1544,1669,1724). Writeback to source systems does not exist: Connector.write is optional (packages/sync/src/connectors/connector.ts:155) and no connector implements it, and sync.writeback is parsed at packages/sync/src/mapping/mapping-parser.ts:262 and read by nothing.

**Gap:** Link properties are still uneditable through any governed path (no updateLink, no generic link mutation), ROLLBACK_ALL still cannot restore a deleted object, and the new direct update/delete mutations are edits without governance — FGA only, no consent gate, no audit record, no field-write policy. Source-system writeback is entirely unimplemented.
...
**Evidence (read 15 Aug):** Row-level: real per-object FGA checks plus listObjects scoping, with fail-closed per-tenant store mapping and no fallback store (authorization-service.ts:39-49; server.ts:623). Column-level: FieldPermissionConfig is loaded from packs and wired (schema-loader.ts:933-1038, server.ts:644), redaction is applied on every read surface (rest/route-generator.ts:553, 797, 915; resolver-generator.ts:342, 377, 460, 686, 791, 1014, 1289; cdm/router.ts:170, 272; functions/invoke-function.ts:108), and the filter/sort oracle is closed on REST list (route-generator.ts:519-537, covering filter fields AND orderBy), GraphQL (resolver-generator.ts:197-210, 745, 1134-1142, 1242-1271), FHIR (fhir/router.ts:191-194) and MCP (mcp-server/src/tools.ts:274-277). STILL ABSENT — cell-level: getVisibleFields(userId, roles, objectType) has no row/object parameter and computes a union of role->field sets (authorization-service.ts:315-343), so field visibility cannot vary per row. STILL ABSENT — write-side field permissions: FieldPermissionConfig models only `visibleFields` (security/src/authz/types.ts:21-47); REST PUT checks the `editor` relation then passes req.body straight to objectManager.update (rest/route-generator.ts:885-910) and GraphQL updateFoo does the same (resolver-generator.ts:988-1008), so a caller who cannot READ a field can still OVERWRITE it. NEW surface hole: GraphQL subscription payloads carry old/new values for changed fields (subscription-manager.ts:29, 80) and are never passed through redactFields — a redacted column leaks on the subscription surface.

**Gap:** No cell-level (per-row) field visibility; no write-side field permissions on either REST or GraphQL update; subscription payloads bypass field redaction entirely.
...
**Evidence (read 15 Aug):** Landed: computed fields are now resolved on the list paths. ObjectManager.query and .search merge them via withComputed in bounded waves (object-manager.ts:349, 387, 415-434), and both REST and GraphQL list/search go through ObjectManager (rest/route-generator.ts:597, 1506; resolver-generator.ts:836, 1326), so computed values are non-null in lists. The evaluator's builtin registry grew to countLinks, lookupField, sumLinks/avgLinks/minLinks/maxLinks (computed-field-evaluator.ts:42-49) with FunctionType dispatch (:273-284), and per-function pack-dir resolution replaced the first-pack pin (function-executor.ts:242, 343 packDirByFunction, wired server.ts:352 from schema-loader functionPackDirs). STILL BROKEN — the ODL validator whitelist was NOT updated: BUILTIN_COMPUTE_FNS = new Set(['countLinks']) (odl/src/validator/index.ts:488) and anything else is a hard error COMPUTED_UNKNOWN_FN (:509-517), so `odl validate` (cli/index.ts:55-89) and `odl apply` (cli/index.ts:160-173, exit 1) reject every schema that uses lookupField, the aggregate builtins, or a FunctionType-backed computed field. The API's schema-loader only calls parseOdl and never validateSchema (schema-loader.ts:950-968), so the compiler and the runtime disagree about what is legal. EAGER/TTL: `cache` and `ttl` are parsed and stored (odl/parser/index.ts:325-332) but getComputedFields filters to LAZY-or-absent (computed-field-evaluator.ts:308-315), so a field declared cache: EAGER is emitted into the SDL/REST shape and never evaluated — silently null, no warning. No filter/orderBy over computed fields (excluded from queryable/groupBy: rest/route-generator.ts:140, 690, 1300) and aggregate is a straight storage pass-through (object-manager.ts:367). Surface divergence: the MCP read tools call deps.storage.queryObjects directly (mcp-server/src/tools.ts:331), bypassing ObjectManager, so computed fields are absent on /mcp while present on REST/GraphQL.

**Gap:** The ODL validator still whitelists only countLinks, so validate/apply reject the very functions the runtime added; cache: EAGER/ttl parse but are silently never evaluated; no filter/orderBy/aggregate over computed fields; MCP bypasses ObjectManager so computed fields are missing there.
...
**Evidence (read 15 Aug):** Re-read TODAY (working tree; note tree is dirty from a concurrent agent, but its diff does not touch any aggregate path — `git diff packages/api/src/rest/route-generator.ts` is three `_actorId` lines only).

WHAT LANDED (9504d9c): packages/api/src/rest/route-generator.ts:1298-1318 builds `aggregatable` from schema fields minus @link/@computed and 400s unknown names across fields/groupBy/buckets/orderBy. Consent IS now applied on the per-type aggregate path on BOTH surfaces (route-generator.ts:1351-1357 and graphql/resolver-generator.ts:1163-1169 both call resolveConsentedIds) — the prior 'consent not applied to aggregates' gap is stale. Bucket interval is allowlisted in the Postgres builder (storage-postgres/src/objects/aggregate.ts:66-72, commit 854b2e8) and in memory (storage-memory/src/memory-storage-provider.ts:771-776). MIN/MAX-on-non-numeric now throws in both (aggregate.ts:204-211; memory 857-861).

BUT THE VALIDATION INTRODUCED A CAPABILITY REGRESSION — ran the built route (node against packages/api/dist/rest/route-generator.js, schema `type Reading{ward,value}`):
  POST /readings/aggregate {fields:[{field:'value',fn:'sum',alias:'total'}],groupBy:['ward'],orderBy:[{field:'total',desc}],limit:5} -> 400 "Cannot aggregate on total: not an aggregatable field of Reading". Same 400 for the default alias `sum_value`. orderBy on a groupBy field -> 200.
Cause: route-generator.ts:1307 folds `orderBy` field names into the same schema-field check, and an aggregate alias is never a schema field. So 'top N groups by the measure' — the core pivot operation — is now unreachable over REST, even though storage implements it: the memory provider sorts on `a.values[sort.field]` (memory-storage-provider.ts:890-904) and returned [{ward:B,total:9},{ward:A,total:6}] when called directly.

STILL DIVERGENT / MISSING:
- GraphQL has no orderBy/limit/offset on aggregate at all — SDL is `${lower}Aggregate(filter, groupBy, buckets, fields)` (packages/odl/src/codegen/index.ts:918) and the resolver builds AggregateQuery with only fields/groupBy/buckets/filter (resolver-generator.ts:1178-1187). No group ordering or pagination on that surface.
- The field-name validation is REST-per-type only. GraphQL's generateAggregateResolver (resolver-generator.ts:1103-1194) has no `aggregatable` check, and engine ObjectManager.aggregate is a pure pass-through (packages/engine/src/objects/object-manager.ts:357-369 "no validation needed for reads"). Verified the divergence is still live in storage: mem.aggregateObjects with field 'nope' returned {groups:[{keys:{},values:{sum_nope:null}}]} — a silent wrong answer where Postgres builds SUM("nope") and raises.
- GET /api/v1/object-sets/:id/aggregate (route-generator.ts:2066-2162) has NEITHER the field validation NOR the consent gate — it only resolves allowedIds and merges filters (2116-2156), so a saved aggregation over a consent-subject type aggregates non-consented records.
- No distinct/percentile/median/stddev/running total: ALLOWED_FNS = count|sum|avg|min|max in both providers (aggregate.ts:83; memory:762). No HAVING — I posted `having:{...}` and it was silently dropped (query reaching objectManager had keys fields,groupBy,buckets,filter,orderBy,limit,offset).
- Aggregation over a traversal-derived set still impossible: ObjectSetDefinition is one objectType + filter + orderBy + limit + one aggregation (packages/spi/src/object-set.ts:12-26); no traversal source, no union/intersect.
- Not exposed on MCP: `grep -rli aggregate packages/mcp-server/src` returns nothing; tools are search_* (tools.ts:98) and traverse_* (tools.ts:445) only.
SDK caveat is not load-bearing here — REST and GraphQL are callable directly.

**Gap:** Ordering groups by an aggregate measure is now rejected by REST (route-generator.ts:1307) and absent from the GraphQL SDL (odl/codegen/index.ts:918), so no top-N-by-measure on any surface; GraphQL also has no limit/offset. Field-name validation exists only on the per-type REST route — GraphQL and the object-set aggregate route still let unknown/@computed names through to a provider-dependent answer (memory: null group, Postgres: error), and the object-set route also skips the consent gate. No distinct/percentile/HAVING/running-total, no aggregation over a traversal-derived or composed set, no MCP surface.
