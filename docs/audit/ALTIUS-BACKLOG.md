...
**Gap:** AGE remains a decorative write-only mirror when it IS enabled (`v.updated = true`, no property sync, nothing reads it) — now opt-out rather than mandatory, but still dead weight for anyone who leaves it on. No variable-depth (`maxDepth`) traversal on either provider; both refuse it rather than truncating.
