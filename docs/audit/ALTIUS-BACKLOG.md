...
> ⚠️ **PARTLY STALE, re-verified 16 Aug 2026.** The per-surface divergence below is CLOSED: the GraphQL aggregate resolver now validates field names and refuses `@computed`/`@link` fields (`resolver-generator.ts:1287`) and blocks redacted fields (`:1310`), and the SDL does expose `orderBy`, `limit` and `offset` (`odl/src/codegen/index.ts:949`), which the resolver forwards. Landed in `9504d9c`. The rest of the gap — no percentile/median, MIN/MAX over DateTime refused by both providers, bucket-key Date-vs-ISO divergence, no UI layer — was not re-checked and stands as written.
...
**Evidence (read 15 Aug):** The agent-facing data half is real; the generative half does not exist. MCP is production-wired behind a pack capability gate: packages/api/src/server.ts:1192-1229 mounts POST/DELETE /mcp via createMcpServer with schema, actionExecutor, authorizationService, authenticator, storage, manifestRegistry and consentSubjectTypes; packages/mcp-server/src/tools.ts:46-60 builds one tool per ActionType plus a search_<Type> per ObjectType, with JSON-Schema inputs derived from @param fields (:64-90), OIDC-authenticated and running the same governed action pipeline with consent defaults (:18-24). DEMOTIONS: (1) ~~there is NO LLM client in the repo~~ — STALE: `AnthropicLLMClient` exists (packages/engine/src/llm/anthropic-llm-client.ts) with real `complete()`/`stream()`; (2) no prompt or generation endpoint exists on REST or GraphQL — the generated Query/Mutation fields (packages/odl/src/codegen/index.ts:740-830) contain no prompt/generate/complete field; (3) no vector or embedding storage — StorageCapabilities (packages/spi/src/ontology.ts:233) has no vector flag and no pgvector appears in the DDL; (4) ~~MCP is a server, not a client~~ — PARTIALLY STALE: `packages/aip-agent` now provides a Deep Agents harness that connects to the MCP endpoint as a client, so Altius can host an agent that calls itself.

**Update (17 Aug):** "NO LLM client" is stale — `AnthropicLLMClient` exists. "MCP is a server, not a client" is partly stale — `packages/aip-agent` provides a reference agent that connects to Altius MCP as a client. Still `partial`: no chat UI widget, no generation endpoint exposed to a widget, no chat/message persistence in the platform.

> **Phase 4 F4.3 update (17 Aug):** Embedding storage and vector similarity search now exist as a dedicated SPI: `EmbeddingStore` with upsert/get/delete/deleteAllForObject/search/count (packages/spi/src/embeddings.ts). `cosineSimilarity` helper is exported from the SPI. `InMemoryEmbeddingStore` implements brute-force cosine similarity with tenant isolation, minScore filtering, limit, and allowedObjectIds authorization filtering (packages/storage-memory/src/in-memory-embedding-store.ts). REST endpoints: PUT/GET/DELETE /api/v1/embeddings/:type/:id/:field, POST /api/v1/embeddings/:type/:field/search (packages/api/src/rest/embedding-routes.ts). 14 embedding tests pass (packages/storage-memory/src/__tests__/embedding-store.test.ts). The existing `LLMClient.embed()` can generate vectors; this store persists and queries them. REMAINING GAPS: no PostgreSQL vector store (pgvector), no automatic embedding generation on object write, no GraphQL vector search queries, no chat UI widget, no chat/message persistence.

**Gap:** Altius can be a tool provider to someone else's chatbot and now ships a reference AIP agent (packages/aip-agent). Embedding storage and cosine similarity search now exist in-memory with REST endpoints. Still absent: no chat UI, no generation endpoint exposed to a widget, no PostgreSQL vector store (pgvector), no automatic embedding generation, no chat/message persistence. AIP Generated Content has no path at all.
...
**Evidence (updated 17 Aug, Phase 4 F4.1):** GeoPoint is a first-class ODL scalar (packages/odl/src/validator/index.ts:23, packages/odl/src/codegen/index.ts:29,:37, packages/storage-postgres/src/schema/type-mapping.ts:25 JSONB, packages/odl/src/codegen/sdk.ts:33 `{lat,lng}`). Spatial query predicates now exist in the SPI: `within` (bounding box), `near` (Haversine radius), and `withinPolygon` (ray-casting polygon containment) (packages/spi/src/ontology.ts FieldPredicate operator union). The in-memory provider evaluates all three: bounding-box range check, Haversine distance via `haversineMeters` helper, and ray-casting via `pointInPolygon` helper (packages/storage-memory/src/memory-storage-provider.ts). The PostgreSQL provider translates `within` to JSONB coordinate extraction + BETWEEN, `near` to inline Haversine SQL, and `withinPolygon` to a LATERAL ray-casting subquery — no PostGIS required (packages/storage-postgres/src/objects/filter-to-sql.ts). Tests: 6 spatial predicate tests covering bounding-box, radius (near), polygon containment, null-location handling, and boundary behavior (packages/storage-memory/src/__tests__/spatial-predicates.test.ts). All pass. REMAINING GAPS: no GeoShape ODL scalar type (polygon/line as a stored property); no spatial index (R-tree/grid); no GraphQL filter input generation for `near`/`withinPolygon` (only `within` is generated); no map UI widget; no tile/geocoding service; GeoPoint validation still accepts any non-null object (validation.ts).

**Gap:** Backend spatial query primitives (bbox, radius, polygon) now exist in both providers, but no GeoShape scalar, no spatial index, no GraphQL filter generation for the new operators, and no map UI widget. Markers can be filtered by viewport/radius/polygon server-side, but the widget layer is still entirely absent.
...
**Evidence (updated 18 Aug, Phase 3 F3.2):** First-class time-series backend now exists: `@timeSeries` ODL directive on Float/Int/String fields (packages/odl/src/parser/types.ts:170), `TimeSeriesStore` SPI with putPoint/getSeries/deleteRange/getLatestPoint (packages/spi/src/time-series.ts), `InMemoryTimeSeriesStore` with time-range filtering, tag filtering, ordering, limit, and bucketing (avg/sum/min/max/count/first/last) (packages/storage-memory/src/in-memory-time-series-store.ts), REST endpoints GET/POST/DELETE /api/v1/{plural}/:id/series/:property with bucketInterval/bucketFunction query params (packages/api/src/rest/timeseries-routes.ts). 17 tests pass. The prior per-object version history substrate remains: temporal reads in SPI (getObjectAtVersion/getObjectAtTime), REST GET /api/v1/{plural}/:id/history.

**Gap:** No time-series widget UI (sparklines, columns, analysis chart). No GraphQL time-series query field. No PostgreSQL time-series store. No multi-object aggregate series. No auto-materialization of latest point on object read. Bucketing is in-memory only.

> **Phase 4 F4.4 update (17 Aug):** Time-series threshold rules and alerting now exist: `AlertingService` SPI with rule CRUD, alert lifecycle (active/acknowledged/resolved), and rule evaluation (packages/spi/src/alerting.ts). `ThresholdRule` supports gt/gte/lt/lte operators, consecutivePoints requirement, minDurationSeconds sustained-condition, and tag filtering. `pointSatisfies` and `findConsecutiveRun` helpers are exported from the SPI. `InMemoryAlertingService` implements full rule and alert management, evaluates rules against time-series points, and dispatches notifications via `NotificationStore` when configured (packages/storage-memory/src/in-memory-alerting-service.ts). REST endpoints: POST/GET/PATCH/DELETE /api/v1/alerting/rules, GET /api/v1/alerting/alerts, POST acknowledge/resolve, POST /api/v1/alerting/evaluate (packages/api/src/rest/alerting-routes.ts). 22 alerting tests pass. This connects time-series ingestion to threshold detection and notification dispatch, but no widget UI exists.
...
**Evidence (updated 18 Aug, Phase 3 F3.4):** `CommentStore` SPI interface with createComment/getComment/listComments/updateComment/deleteComment/setResolved plus getNotifications/markNotificationRead (packages/spi/src/comments.ts). `parseMentions` utility extracts @userId patterns from Markdown body (packages/spi/src/comments.ts:113). `InMemoryCommentStore` implements full thread lifecycle: thread roots and replies, @-mention parsing on create, mention notifications to mentioned users (excluding self), reply notifications to parent comment author, resolve/unresolve, edit tracking, pagination, and filtering by thread/resolved/author (packages/storage-memory/src/in-memory-comment-store.ts). REST endpoints: GET/POST /api/v1/{plural}/:id/comments, PUT/DELETE /api/v1/comments/:commentId, POST /api/v1/comments/:commentId/resolve|unresolve, GET /api/v1/notifications, POST /api/v1/notifications/:id/read (packages/api/src/rest/comment-routes.ts). Tests: 18 tests covering mention parsing, thread creation, reply notifications, self-mention exclusion, listing/filtering, edit tracking, resolve/unresolve, delete, notification read/unread filtering (packages/storage-memory/src/__tests__/comment-store.test.ts). All pass.

**Gap:** No PostgreSQL comment store. No GraphQL comment queries/mutations. No comment UI widget. No action-log mirroring. No email/webhook notification transport. No permission inheritance from commented object. No rich-text rendering beyond plain Markdown.

> **Phase 4 F4.2 update (17 Aug):** A platform-wide `NotificationStore` SPI now exists (packages/spi/src/notifications.ts) with typed notifications (mention, reply, action, alert, assignment, system, branch, sync), user preferences (per-type channel selection, email/push enable/disable, muted types), and effective-channel resolution. `InMemoryNotificationStore` implements full CRUD, list with filtering, mark-read/mark-all-read, preferences, and tenant isolation (packages/storage-memory/src/in-memory-notification-store.ts). REST endpoints: GET/POST /api/v1/notifications, mark-read, mark-all-read, delete, GET/PUT preferences (packages/api/src/rest/notification-routes.ts). The action side-effect executor now supports a `notification` side-effect type via `NotificationDispatcher` (packages/actions/src/sideeffects/types.ts, side-effect-executor.ts). 12 notification store tests pass. This elevates notifications from comment-only to platform-wide, but the comment widget UI, email/push transport, and GraphQL notification queries remain absent.
...
**Evidence (Phase 10):** `EmbeddingService` SPI (packages/spi/src/embedding.ts) defines a registered app catalog (workshop/quiver/notepad/vertex/custom/external kinds with URL, embeddable flag, allowed embed origins, deep-link patterns, command support), embedding manifests (CSP frame-src, framing allowed, context parameters, emitted events, accepted commands), cross-app commands (source/target app, payload, status lifecycle: pending/delivered/processed/failed), and app pairing (shared state keys, bidirectional sync, active flag). `InMemoryEmbeddingService` (packages/storage-memory/src/in-memory-embedding.ts) implements all operations. 7 tests in phase10-widget-services.test.ts.

**Gap:** No iframe rendering. No CSP header integration. No REST/GraphQL routes. No persistent storage. No real-time command delivery (polling only). No actual app hosting.
...
**Gap:** No UI rendering (Tabs, Stepper, Markdown, Navbar, Header are frontend-only). No multipart upload transport. No REST/GraphQL routes. No persistent storage. No geolocation validation against GeoPoint schema.
...
**Evidence (updated 18 Aug, Phase 3 F3.1):** `Attachment` scalar type now exists in ODL (packages/odl/src/validator/index.ts:25), codegen (packages/odl/src/codegen/index.ts:34), engine validation (packages/engine/src/objects/validation.ts:155), and Postgres JSONB mapping (packages/storage-postgres/src/schema/type-mapping.ts:27). `BlobStore` SPI interface (packages/spi/src/blob-store.ts) with put/get/delete/exists. `InMemoryBlobStore` with SHA-256 content-addressed deduplication (packages/storage-memory/src/in-memory-blob-store.ts). REST upload/download/delete at /api/v1/attachments with raw body parsing via express.raw({ limit: '50mb' }) (packages/api/src/rest/attachment-routes.ts). 19 tests pass across ODL, engine, and storage.

**Gap:** No media widget UI (preview, uploader, PDF viewer, image annotation, spreadsheet display, video/audio). No PostgreSQL/disk blob store. No thumbnail generation. No GraphQL upload mutation. No consent/redaction integration for media content.
...
**Evidence (Phase 10):** `WidgetLibraryService` SPI (packages/spi/src/widget-library.ts) defines a widget registry (type, name, category: data/chart/filter/input/layout/action/media/collaboration/ai, config schema, data sources, live updates, min width), app definitions (pages → sections → widget instances with data bindings, variables, theme), and CRUD for pages/widgets (add/update/remove with version increment). `InMemoryWidgetLibraryService` (packages/storage-memory/src/in-memory-widget-library.ts) ships with 22 pre-registered widget definitions (object_table, object_list, object_view, chart_xy, chart_pie, pivot_table, metric_card, filter_list, object_selector, text_input, number_input, date_picker, button_group, stepper, tabs, markdown, map, gantt, time_series, media_preview, comments, aip_chat). 10 tests in phase10-widget-services.test.ts.

**Gap:** No UI rendering. No drag-and-drop editor. No widget preview. No REST/GraphQL routes. No persistent storage. No variable evaluation engine. No template library.
...
**Evidence (Phase 10):** `PlatformResourceService` SPI (packages/spi/src/platform-resources.ts) defines a resource catalog (file/project/folder/dataset/notebook/report/model kinds with hierarchy, path, MIME type, size, storage ref, tags), resource-to-object links (attachment/reference/source/output/annotation kinds), browse (by path hierarchy), search (by name/tag), and upload-and-link (creates a resource and links it to an object in one operation). `InMemoryPlatformResourceService` (packages/storage-memory/src/in-memory-platform-resources.ts) implements all operations with cascading link deletion. 9 tests in phase10-widget-services.test.ts.

**Gap:** No actual blob storage integration. No multipart upload transport. No REST/GraphQL routes. No persistent storage. No file content streaming. No FGA/consent integration on resources.
...
**Evidence (updated 17 Aug, Phase 5 F5.4):** `OntologyUsageMetricsService` SPI now exists with `record`, `getObjectTypeMetrics`, `getActionFunctionMetrics`, `getSummary`, `queryEvents`, `getActiveUserCount`, and monitoring rule CRUD/evaluation (packages/spi/src/usage-metrics.ts). `OntologyUsageEvent` tracks tenantId, userId, operation type (read/create/update/delete/search/aggregate/action/function/link/subscription), objectType, objectId, actionOrFunctionName, success, durationMs, and timestamp. `ObjectTypeMetrics` includes per-type reads/writes/searches/aggregates/actions/functions/errors, activeUsers, avgDurationMs, and p95DurationMs. `ActionFunctionMetrics` includes per-action/function executions, errors, activeUsers, and duration stats. `UsageMonitoringRule` supports error_rate/avg_duration/p95_duration/request_count metrics with gt/gte/lt/lte operators and configurable time windows. `InMemoryOntologyUsageMetricsService` implements full event recording, aggregation, querying, and monitoring rule evaluation (packages/storage-memory/src/in-memory-usage-metrics.ts). 10 usage metrics tests pass. REMAINING GAPS: not wired into the actual API request pipeline (events must be recorded manually), no PostgreSQL metrics store, no REST/GraphQL endpoint to query metrics, no 30-day rolling window aggregation (queries are ad-hoc), no change-impact analysis.

**Gap:** Ontology usage metrics service exists with per-type/per-action metrics, active user tracking, monitoring rules, and event querying. Still absent: API pipeline integration, PostgreSQL store, REST/GraphQL endpoints, 30-day windowing, change-impact analysis.
...
**Gap:** No UI drag-and-drop rendering. No real-time drag event delivery. No REST/GraphQL routes. No persistent storage. No actual media payload transfer.
...
**Evidence (Phase 12):** `WorkshopPlatformService` SPI (packages/spi/src/workshop-platform.ts) defines `WorkshopAppDefinition` (pages, sections with layouts: stack/grid/tabs/columns/sidebar/loop, header, overlays: modal/drawer/popover/toast, theme, module interface, version), page/section/widget CRUD, overlays CRUD, app templates (category, tags, partial app definition), and createAppFromTemplate. `InMemoryWorkshopPlatformService` implements all operations with version increment on each update. 6 tests in phase12-workshop-platform.test.ts.

**Gap:** No UI editor. No drag-and-drop layout engine. No app definition rendering. No REST/GraphQL routes. No persistent storage. No example apps shipped.
...
**Gap:** No mobile UI. No mobile launcher rendering. No QR/location widget rendering. No browser history integration. No REST/GraphQL routes. No persistent storage.
...
**Evidence (Phase 12):** `WorkshopPlatformService` SPI defines `AppModule` (name, interface with inputs/outputs, sections, published flag, version), `ModuleInterface` (typed inputs with required/defaultValue, typed outputs), `ModuleInstance` (input/output bindings), loop layout config (loopConfig with variableName/itemVariableName), and embedModule operation (binds a module into a section with input bindings). `InMemoryWorkshopPlatformService` implements module CRUD, publish, and embed with input bindings. 2 tests in phase12-workshop-platform.test.ts.

**Gap:** No UI module rendering. No loop layout rendering. No URL/deep-link initialization. No REST/GraphQL routes. No persistent storage. No module marketplace.
...
**Evidence (Phase 12):** `WorkshopPlatformService` SPI defines `ReactiveVariable` (name, type: string/number/boolean/date/object_set/object/array/struct/aggregation, source, lazy flag, transformations, structFields), `VariableSource` (kind: static/function/aggregation/object_property/object_set/expression with dependencies), `VariableTransformation` (map/filter/sort/reduce/format/lookup/math/string_op), and `VariableLineage` (dependsOn/dependedBy). `InMemoryWorkshopPlatformService` implements variable CRUD, lineage graph computation from dependency declarations, and evaluateVariable (returns static values). 8 tests in phase12-workshop-platform.test.ts.

**Gap:** No reactive runtime engine. No actual source evaluation (function/aggregation/object_set sources return null). No transformation execution. No recompute scheduler. No REST/GraphQL routes. No persistent storage. No UI variable editor.
...
**Evidence (Phase 12):** `WorkshopPlatformService` SPI defines `WidgetCatalogEntry` with display optimization metadata (supportsVirtualization, supportsColumnResize, supportsFrozenColumns, supportsDensityModes, defaultPageSize) and mobileOptimized flag. `InMemoryWorkshopPlatformService` ships with 60 pre-registered widget definitions across 12 categories: data (object_table, object_list, object_view, property_list, object_set_title, links, tree_view, kanban), chart (chart_xy, chart_pie, chart_bar, chart_vega, pivot_table, metric_card, waterfall, observability_chart, heatmap, scatter_plot), filter (filter_list, object_selector, date_picker, date_range, search_bar, user_select), input (text_input, number_input, checkbox, radio_group, dropdown), layout (tabs, stepper, markdown, header, spacer, divider, progress_bar, badge, tooltip, accordion), action (button_group, action_form), media (media_preview, media_uploader, pdf_viewer, image_annotation, spreadsheet_display, video_player, audio_player), collaboration (comments, action_log), ai (aip_chat, aip_generated_content), navigation (mobile_navbar, breadcrumb), time (time_series, gantt, timeline, calendar), geo (map, current_location). 5 tests in phase12-workshop-platform.test.ts.

**Gap:** No UI rendering for any widget. No REST/GraphQL routes. No persistent storage. No per-widget display optimization runtime. No widget configuration UI.
...
**Evidence (updated 17 Aug, Phase 5 F5.3):** `JustificationStore` SPI now exists with create/get/list/approve (packages/spi/src/security-governance.ts). `JustificationRecord` captures tenantId, userId, actionName, objectType, objectId, justification text, category (break-glass/routine/audit/emergency/legal), approval state, and timestamps. `InMemoryJustificationStore` implements full CRUD with filtering by user/action/object/time and tenant isolation (packages/storage-memory/src/in-memory-security-governance.ts). 5 justification tests pass. REMAINING GAPS: not wired into the action execution pipeline (actions don't yet require justification before executing), no GraphQL/REST endpoint to submit justifications, no per-action "requires justification" declaration in ODL, justification not persisted into audit records.

**Gap:** Justification capture store exists with categories, approval, and querying. Still absent: action pipeline integration, per-action justification requirement declaration, REST/GraphQL endpoints, audit record integration.
...
> ✅ **RE-VERIFIED against source, 17 Aug 2026 (Phase 5 F5.3).** absent → partial. Marking propagation SPI and scoped sessions exist; no implementation wired into the data plane.

**Evidence (updated 17 Aug, Phase 5 F5.3):** `MarkingPropagationService` SPI now exists with `computeEffectiveMarkings`, `getRules`, `setRules`, and `simulate` (packages/spi/src/security-governance.ts). `MarkingPropagationRule` supports source/target types, link paths, propagate flag, and `stopPropagating`. `PropagatedMarkings` returns direct, inherited, and effective markings with lineage paths. The existing `MarkingPolicy` (packages/security/src/markings/marking-policy.ts) provides the marking definitions and categories that propagation operates over. Provenance writes exist (Postgres only, packages/engine/src/objects/object-manager.ts). REMAINING GAPS: no `MarkingPropagationService` implementation (SPI only), no integration with the object read/write pipeline, no lineage graph traversal (FieldProvenance is flat per-field), no REST/GraphQL endpoint, `stop_propagating` declared in SPI but not enforced.

**Gap:** Marking propagation SPI with rules, simulation, and effective-marking computation exists. Still absent: implementation, data-plane integration, lineage graph traversal, REST/GraphQL endpoint, stop_propagating enforcement.
...
**Evidence (updated 17 Aug, Phase 5 F5.3):** `AccessExplanationService` SPI now exists with `explain()` that returns `AccessExplanation` containing granted/denied, resource, userId, and detailed `AccessExplanationReason[]` (packages/spi/src/security-governance.ts). `DefaultAccessExplanationService` implements the service by checking tenant isolation, ReBAC relations (via `AuthorizationService.check`), marking policy, and consent, producing a human-readable summary (packages/security/src/authz/access-explanation-service.ts). Each reason includes the check type (role/rebac_relation/marking/consent/tenant), whether it passed, detail text, and the rule evaluated. REMAINING GAPS: no REST/GraphQL endpoint to request explanations, no simulation for another principal (only the calling user), no denial-reason surfacing on reads (still only post-hoc _redactedFields), no marking policy integration in the explanation (placeholder).

**Gap:** Access explanation service exists with per-check reasoning. Still absent: REST/GraphQL endpoint, simulation for other principals, marking policy integration, denial-reason surfacing on read responses.

### `security-gov/scoped-sessions-session-restricted-marking-s` — Scoped sessions (session-restricted marking subsets)

**Status:** `partial`

**Evidence (updated 17 Aug, Phase 5 F5.3):** `ScopedSessionStore` SPI now exists with create/get/getActiveForUser/list/revoke/isMarkingAllowed (packages/spi/src/security-governance.ts). `ScopedSession` carries allowedMarkings, excludedMarkings, label, expiry, revocation state, and creator. `InMemoryScopedSessionStore` implements full session lifecycle with expiry checking, revocation, marking-allowed checks, and tenant isolation (packages/storage-memory/src/in-memory-security-governance.ts). 7 scoped session tests pass. REMAINING GAPS: not wired into the authentication/authorization pipeline (sessions exist but aren't enforced on requests), no REST/GraphQL endpoint to create/manage sessions, no OIDC claim integration, no admin allowlisting.

**Gap:** Scoped session store exists with marking subsets, expiry, and revocation. Still absent: auth pipeline integration, REST/GraphQL endpoints, OIDC claim integration, admin allowlisting.
...
**Evidence (updated 17 Aug, Phase 4 F4.4):** Time-series type and store now exist (Phase 3 F3.2): `@timeSeries` ODL directive (packages/odl/src/parser/types.ts:170), `TimeSeriesStore` SPI with putPoint/getSeries/deleteRange/getLatestPoint (packages/spi/src/time-series.ts), `InMemoryTimeSeriesStore` with bucketing (packages/storage-memory/src/in-memory-time-series-store.ts). Threshold definitions and alert-on-data now exist (Phase 4 F4.4): `AlertingService` SPI with `ThresholdRule` supporting gt/gte/lt/lte operators, consecutivePoints, minDurationSeconds, and tagFilter (packages/spi/src/alerting.ts). `InMemoryAlertingService` evaluates rules against time-series points and generates `Alert` records with active/acknowledged/resolved lifecycle, dispatching notifications via `NotificationStore` (packages/storage-memory/src/in-memory-alerting-service.ts). REST endpoints for rules, alerts, and evaluation (packages/api/src/rest/alerting-routes.ts). 22 alerting tests + 17 time-series tests pass. Per-object version history remains (temporal-queries.ts, /history route). REMAINING GAPS: no PostgreSQL time-series or alerting store, no automatic evaluation on point ingestion (must call evaluate API), no event-bus publication of threshold crossings, no anomaly detection (only static thresholds), no Vertex-style event UI.

**Gap:** Time-series storage, threshold rules, alerting, and notification dispatch exist as backend services. Still absent: PostgreSQL stores, automatic evaluation on ingestion, event-bus publication of crossings, anomaly detection, and any UI.
...
**Evidence (updated 18 Aug, Phase 3 F3.2):** `@timeSeries` ODL directive added to parser (packages/odl/src/parser/index.ts:401) and types (packages/odl/src/parser/types.ts:170 — TimeSeriesDirective with retention and defaultBucket). Validation enforces type must be Float/Int/String and rejects @primary/@link/@computed/@reducer/@unique conflicts (packages/odl/src/validator/index.ts:222 — validateTimeSeriesField, TIMESERIES_INVALID_TYPE/TIMESERIES_CONFLICT). Codegen excludes @timeSeries fields from stored properties (packages/odl/src/codegen/index.ts:85 — isComputedField). `TimeSeriesStore` SPI interface with putPoint/putPoints/getSeries/deleteRange/getLatestPoint (packages/spi/src/time-series.ts). `InMemoryTimeSeriesStore` with time-range filtering, tag filtering, ordering, limit, and bucketing (avg/sum/min/max/count/first/last) at configurable intervals (1s/1m/1h/1d/1w) (packages/storage-memory/src/in-memory-time-series-store.ts). REST endpoints: GET/POST/DELETE /api/v1/{plural}/:id/series/:property with start/end/limit/order/bucketInterval/bucketFunction/tags query params (packages/api/src/rest/timeseries-routes.ts). Tests: 7 ODL directive tests (packages/odl/src/__tests__/timeseries-directive.test.ts), 10 store tests covering range/order/limit/bucketing/tags/delete/latest (packages/storage-memory/src/__tests__/timeseries-store.test.ts). All 17 pass.

**Gap:** No PostgreSQL time-series store (no hypertable or dedicated table). No retention enforcement. No interpolation for missing points. No multi-object aggregate series. No GraphQL time-series query field. No time-series widget UI. Object property does not auto-materialize latest point on read.
...
**Evidence (updated 17 Aug, Phase 4 F4.4):** Standing-rule definition model and alert artifacts now exist: `AlertingService` SPI with `ThresholdRule` (gt/gte/lt/lte operators, consecutivePoints, minDurationSeconds, tagFilter), `Alert` lifecycle (active/acknowledged/resolved), and rule evaluation against time-series points (packages/spi/src/alerting.ts). `pointSatisfies` and `findConsecutiveRun` helpers exported from SPI. `InMemoryAlertingService` evaluates rules and dispatches notifications via `NotificationStore` (packages/storage-memory/src/in-memory-alerting-service.ts). REST endpoints for rule CRUD, alert management, and evaluation (packages/api/src/rest/alerting-routes.ts). 22 alerting tests pass. REMAINING GAPS: no interval/anomaly detection (only static threshold rules), no automatic evaluation on point ingestion, no binding from detection to action execution, no PostgreSQL alerting store, no Foundry Rules TS board UI.

**Gap:** Threshold rules, alert lifecycle, and notification dispatch exist as backend services. Still absent: anomaly/interval detection, automatic evaluation on ingestion, action binding from detections, PostgreSQL store, and TS board UI.

### `analytics-ts/time-series-transform-and-summarizer-engine` — Time series transform and summarizer engine

**Status:** `partial`

**Evidence (updated 17 Aug, Phase 5 F5.5):** Time-series transform and summarizer functions now exist in the SPI (packages/spi/src/ts-transforms.ts): `resample` (downsample to coarser intervals with avg/sum/min/max/count/first/last), `rollingAggregate` (moving window with avg/sum/min/max/std), `lag` (value shift by n positions), `diff` (consecutive point differences), `forwardFill` (carry last known value forward), `linearInterpolate` (interpolate between points at regular intervals), `exponentialSmoothing` (alpha-weighted smoothing), series arithmetic (`addSeries`/`subtractSeries`/`multiplySeries`/`divideSeries` matching by timestamp), and `summarize` (statistical summary: count/mean/median/std/min/max/range/first/last/sum). All functions operate on `TimeSeriesPoint[]` from the existing time-series store. 19 transform tests pass (packages/spi/src/__tests__/ts-transforms.test.ts). REMAINING GAPS: no REST/GraphQL endpoint to invoke transforms, no transform DAG/expression composition layer, no unit conversion, no PostgreSQL-side transforms (all in-memory), no integration with the time-series REST routes.

**Gap:** Transform and summarizer functions exist (resample, rolling, lag, diff, interpolation, smoothing, arithmetic, statistics). Still absent: REST/GraphQL endpoints, transform composition DAG, PostgreSQL-side transforms, unit conversion.
...
> ✅ **UPDATED 16 Aug 2026 — geo data layer landed (`b049a61`).** absent → partial. Geotime/time-series remains absent.

**Update (16 Aug 2026 — landed):** GeoPoint is now validated as real coordinates — numeric lat∈[-90,90], lng∈[-180,180] (the stricter validator kept by merge `94b37cf` supersedes the earlier one; packages/engine/src/objects/validation.ts) — so `{}`/`[1,2]` are rejected. A spatial bounding-box operator `within` was added to FieldPredicate (packages/spi/src/ontology.ts) and implemented on both providers with matching semantics: memory range-checks the point (packages/storage-memory/src/memory-storage-provider.ts), Postgres extracts lat/lng from the JSONB column and range-checks each axis with no PostGIS (packages/storage-postgres/src/objects/filter-to-sql.ts). Both providers now report supportsGeoQueries: true. Reachable from GraphQL via `GeoPointFilter { within: GeoBoundingBoxInput }` (packages/odl/src/codegen/index.ts + resolver mapFilterOp). Tests: engine GeoPoint validation, memory `within`, Postgres filter-to-sql, odl codegen. STILL ABSENT: radius/polygon predicates, a spatial index (needs a PostGIS decision), and geotime/time-series (no series property type or series API). No index → bbox is a full scan with per-row JSONB extraction.
...
**Evidence (updated 18 Aug, Phase 3 F3.1):** `Attachment` scalar added to ODL BUILTIN_SCALARS (packages/odl/src/validator/index.ts:25), SCALAR_MAP and CUSTOM_SCALARS (packages/odl/src/codegen/index.ts:34,:41), engine validation (packages/engine/src/objects/validation.ts:155 — isValidAttachment checks blobId/filename/contentType/size), and Postgres JSONB mapping (packages/storage-postgres/src/schema/type-mapping.ts:27). `BlobStore` SPI interface with put/get/delete/exists (packages/spi/src/blob-store.ts), `InMemoryBlobStore` with SHA-256 deduplication (packages/storage-memory/src/in-memory-blob-store.ts), REST upload/download/delete endpoints at /api/v1/attachments (packages/api/src/rest/attachment-routes.ts). Tests: 5 ODL parser/validator tests (packages/odl/src/__tests__/attachment-scalar.test.ts), 7 engine validation tests (packages/engine/src/__tests__/attachment-validation.test.ts), 7 blob store tests (packages/storage-memory/src/__tests__/blob-store.test.ts). All 19 tests pass.

**Gap:** No PostgreSQL/disk-based blob store implementation (only in-memory). No thumbnail generation. No GraphQL attachment upload mutation. No consent/redaction integration for media. No media preview widget UI.
...
**Update (16 Aug):** `asOfTime` is now honoured by BOTH providers on `queryObjects`, so an as-of collection query exists. Memory rebuilds each object's newest version at or before the instant from its version history; Postgres reads the history table with `DISTINCT ON ("_id") ... WHERE "_updated_at" <= $n ORDER BY "_id", "_version" DESC` as the query source, so the filter, ordering and pagination all run against the historical rows rather than current ones — "who was DISCHARGED last Tuesday" evaluates the predicate against Tuesday's values. An object created after the instant is absent; one deleted before it comes back carrying `_deletedAt` and is dropped by the normal soft-delete clause. `rowToObject` now strips `_history_id`/`_history_created_at` so an as-of row maps to the same shape as a live one. `asOfVersion` is REFUSED on collection queries by both providers with the same message: version numbers are per-object, so "every Patient at version 3" has no answer for a patient that only reached version 2 — refusing beats inventing a reading, because an as-of query that silently returns current rows looks exactly like real historical data. Tests: 5 shared conformance cases in tests/spi-conformance/src/categories/temporal.ts; 4 fail against the pre-change memory provider. **The Postgres half did not run** — `PG_TEST_URL` is unset and the repo still has zero CI workflow runs, so the Postgres implementation rests on review, not on a passing test.
...
> **RE-VERIFIED 17 Aug 2026 — still partial — two gaps closed.** GraphQL execution and set algebra now exist; per-user/group sharing and the SEC-14 execute-path check remain open.

**Evidence (read 17 Aug):** Real and wired in both providers: packages/api/src/server.ts:718-720 selects PostgresObjectSetStore(storage.pool) or InMemoryObjectSetStore and builds ObjectSetManager; packages/storage-postgres/src/object-sets/postgres-object-set-store.ts:42,53-75 self-initialises the _object_sets table; packages/engine/src/object-sets/in-memory-object-set-store.ts:11 mirrors it. REST is complete: packages/api/src/rest/route-generator.ts:207 registers CRUD at :1103/:1123/:1155/:1192/:1223, execute at :1246 (auth-filtered, redacted, consent-paginated, plus ?format=ndjson at :1358) and aggregate at :1391. ~~GraphQL exposes definitions only~~ — CLOSED (17 Aug): codegen now emits `executeObjectSet(id, input): ObjectSetExecutionResult!`, `executeObjectSetAggregate(id): AggregateResult!`, and `combineObjectSets(input): ObjectSet!`; resolvers are wired in resolver-generator.ts. ~~No set algebra~~ — CLOSED (17 Aug): `SetAlgebraInput` SPI type + `ObjectSetManager.combine()` implements UNION (OR of filters), INTERSECT (AND of filters), and DIFFERENCE (left AND NOT right). 4 tests in object-set-algebra.test.ts. STILL OPEN: (a) sharing is one boolean: isPublic=tenant-wide vs creator-only; no grant to a user, group or role. (c) SEC-14 predicate check is missing on the execute path: the REST execute handler never calls getVisibleFields. (e) create does no input validation.

**Gap:** No per-user/group sharing or ACL; execute leaks predicates over redacted fields (SEC-14); create does no input validation. GraphQL execution and set algebra are now closed.
