...
**Gap:** Both self-link and MANY_TO_ONE are proven on memory and UNPROVEN on Postgres — `PG_TEST_URL` is unset locally and the repository has zero CI workflow runs, so the Postgres half of the conformance suite has never executed at all. This is the blocking item for this row and it is an infrastructure gap, not a code one: no amount of test-writing closes it until the Postgres suite actually runs somewhere.
...
**Gap:** `getObjectAtTime` is still not exposed on any API surface (REST/GraphQL/MCP/SDK), and there is no link history API. `asOfTime` is honoured at the SPI but no route or resolver passes it yet, so it is reachable from platform code only. The Postgres as-of path is unverified by execution.
