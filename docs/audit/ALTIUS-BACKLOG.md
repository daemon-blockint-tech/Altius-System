...
**Gap:** None for this row's original two gaps: reads are audited on all three surfaces (REST every read route, GraphQL single-object/list/search/aggregate/traverse, MCP both read tools), and the GraphQL audit read surface pages in the store. Remaining, and belonging to other rows: MCP records an agent as `actor.type: 'user'`, matching what its action path already writes, so agent traffic is still indistinguishable in the trail (see `security-gov/ai-agent-write-governance`); FHIR and CDM read routes are not audited; and read auditing is one record per request, so it evidences that a query ran, not which specific rows it returned.
