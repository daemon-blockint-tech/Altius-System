...
# [](<#get-results-sql-query>)Get Results Sql Query

GET/api/v2/sqlQueries/{sqlQueryId}/getResults

Gets the results of a query. Results are returned in the `serializationFormat` specified at execute time (defaulting to [Apache Arrow](<https://arrow.apache.org/>) if no format is provided).
...
`1 2 3 ``curl \ \t-H "Authorization: Bearer $TOKEN" \ "https://$HOSTNAME/api/v2/sqlQueries/OikKBfqBaWRjOGQ3YTJkNzMtYjhiZi00ZDMzLTlkOWMtOGQzOW/getResults"`
...
**Parameters**| ``  
`GetResultsSqlQueryPermissionDenied`| **Error Code**| `PERMISSION_DENIED`  
**Status Code**|  403  
