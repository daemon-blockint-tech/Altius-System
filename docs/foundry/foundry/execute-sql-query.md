...
`1 2 3 4 5 ``curl -X POST \ \t-H "Content-type: application/json" \ \t-H "Authorization: Bearer $TOKEN" \ "https://$HOSTNAME/api/v2/sqlQueries/execute" \ -d '{"fallbackBranchIds":["master"],"serializationFormat":"CSV","query":"SELECT * FROM `/Path/To/Dataset`"}'`
