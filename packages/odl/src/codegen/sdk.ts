...
function generateObjectAccessor(obj: ObjectType): string {
  ...
  return [
    ...
    `        this.query<${name} | null>(\`query { ${lower}(id: "\${id}") { ${fields} } }\`),`,
    '',
    `      list: (filter?: ${name}Filter, pagination?: PaginationArgs): Promise<${name}Connection> =>`,
    `        this.query<${name}Connection>(\`query($filter: ${name}Filter, $first: Int, $after: String) { ${lower}s(filter: $filter, first: $first, after: $after) { edges { node { ${fields} } cursor } pageInfo { hasNextPage hasPreviousPage startCursor endCursor } totalCount } }\`, { filter, first: pagination?.first, after: pagination?.after }),`,
    '',
    `      onChange: (id: string, callback: (event: ChangeEvent<${name}>) => void): Subscription =>`,
    `        this.subscribe<${name}>('${name}', id, '${fields}', callback),`,
    '',
    `      onAnyChange: (callback: (event: ChangeEvent<${name}>) => void, filter?: ${name}Filter, onClose?: () => void): Subscription =>`,
    `        this.subscribe<${name}>('${name}', null, '${fields}', callback, filter as Record<string, unknown> | undefined, onClose),`,
    '    };',
    '  }',
  ].join('\n');
}
